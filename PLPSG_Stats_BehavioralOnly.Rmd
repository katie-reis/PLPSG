...
### Analysis 3 - long term recovery effect on PLPSG dataset
```{r}
...
### anovas including learning and pre-test score as covariates (in separate models) - DV is protective, between subject variable is condition
```{r}
...
fit_consolidation_pretestcov = aov(protective ~ block1 + condition, data = PLPSG_sleep_stats_wider)
pander(summary(fit_protective_pretestcov), style = "rmarkdown")
```

### including undergrad status as variable in ANOVA, DV is protective, between subject variable is condition
```{r}
#include undergrad
