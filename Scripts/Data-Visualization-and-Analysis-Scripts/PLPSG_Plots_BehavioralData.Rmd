...
```
### visualize with a lineplot the PLPSG behavioral data all tests
```{r}
ggplot(PLPSG_sleep_stats_longer, aes(x = block, y = score, color = condition, group = condition)) +
  stat_summary(fun = mean, geom = "line", size = 0.5, position = position_dodge(width = 0.7)) +
  stat_summary(fun = mean, geom = "point", size = 1.5, position = position_dodge(width = 0.7)) +
  stat_summary(fun.data = mean_se, geom = "errorbar", width = 0.15, position = position_dodge(width = 0.7)) +
  geom_point(position = position_jitterdodge(dodge.width = 0.7, jitter.width = 0.35, jitter.height = 0), 
             alpha = 0.6, size = 1) +
  scale_color_manual(values = c("sleep" = "deepskyblue4", "wake" = "indianred3")) +
  theme_minimal() +
  scale_x_discrete(label = c("Pretest", "Posttest1", "Posttest2", "Posttest3", "Posttest4")) + 
  ...
  scale_color_manual(
  name = "Condition",  # This sets the legend title
  values = c("sleep" = "deepskyblue4", "wake" = "indianred3"),
  labels = c("sleep" = "nap", "wake" = "wake")  # This renames the legend entries
)
...
#relationship between learning and loss
```{r}
fit = lm(as.numeric(loss) ~ as.numeric(learning), subset(PLPSG_sleep_stats_wider, condition == "sleep"))

ggplot(data = subset(PLPSG_sleep_stats_wider, condition == "sleep")) + 
  aes(x = learning, y = loss,
      na.rm = TRUE) +
  xlab("learning") + 
  ylab("loss") +
  theme(axis.title.x = element_text(size=9), axis.title.y = element_text(size=9)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(
        aes(x = 0.25,  # Position near the first category
            y = 0.2,  # Above the data
            label = paste("p =", signif(summary(fit)$coef[2, 4], 5))),
        inherit.aes = FALSE,
        size = 2.5)

```

#relationship between loss and recovery
```{r}
fit = lm(as.numeric(recovery) ~ as.numeric(loss), subset(PLPSG_sleep_stats_wider, condition == "sleep"))

ggplot(data = subset(PLPSG_sleep_stats_wider, condition == "sleep")) + 
  aes(x = loss, y = recovery,
      na.rm = TRUE) +
  xlab("loss") + 
  ylab("recovery") +
  theme(axis.title.x = element_text(size=9), axis.title.y = element_text(size=9)) +
  ...
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(
        aes(x = 0.2,  # Position near the first category
