...
ggplot(data = subset(PLPSG_sleep_stats_wider, condition == "sleep")) + 
  aes(x = as.numeric(Spindles_N2_Count), y = recovery,
      na.rm = TRUE) + 
  xlab("Count of N2 Spindles") + 
  ylab("Recovery") +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(aes(x =500, y = 0.035), 
               hjust = 0, size = 4.5, 
               label = paste("p =", round(summary(fitn2)$coef[2, 4], 2)), 
               inherit.aes = FALSE) + 
...
ggplot(data = subset(PLPSG_sleep_stats_wider, condition == "sleep")) + 
  aes(x = as.numeric(Spindles_N3_Count), y = recovery,
      na.rm = TRUE) + 
  xlab("Count of SWS Spindles") + 
  ylab("Recovery") +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(aes(x =540, y = 0.04), 
               hjust = 0, size = 4.5, 
               label = paste("p =", round(summary(fitn3)$coef[2, 4], 2)), 
               inherit.aes = FALSE) + 
  theme(text = element_text(family = "serif"),
        axis.title = element_text(size = 17),
        axis.text = element_text(size = 14))
...
ggplot(data = subset(PLPSG_sleep_stats_wider, condition == "sleep")) + 
  aes(x = as.numeric(Spindles_N2andN3_Count), y = recovery,
      na.rm = TRUE) + 
  xlab("Total Spindle Count (N2 and SWS)") + 
  ylab("Recovery") +
  geom_point(size = 2) +
  geom_smooth(method = "lm", se = FALSE) +
  geom_label(aes(x =660, y = 0.04), 
               hjust = 0, size = 4.5, 
               label = paste("p =", round(summary(fitn2andn3)$coef[2, 4], 2)), 
               inherit.aes = FALSE) + 
  theme(text = element_text(family = "serif"),
        axis.title = element_text(size = 17),
        axis.text = element_text(size = 14))
...
n3fullcycle = ggplot(PLPSG_sleep_stats_wider, aes(x = as.numeric(N3), y = recovery, color = FullCycle)) +
  ...
  scale_color_manual(name = "Full Cycle",  # legend title
    values = c("FullCycle" = "darkseagreen4", "NoFullCycle" = "rosybrown3"),
    labels = c("FullCycle" = "Yes", "NoFullCycle" = "No")) +
  labs(
    x = "Time in SWS (minutes)",
    y = "Recovery"  ) +
  theme_minimal() +
...
ggplot(PLPSG_sleep_stats_wider, aes(x = as.numeric(R), y = recovery, color = FullCycle)) +
  geom_point(size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", se = TRUE) +
  scale_color_manual(
    name = "Full Cycle",  # legend title
    values = c("FullCycle" = "darkseagreen4", "NoFullCycle" = "rosybrown3"),
    labels = c("FullCycle" = "Yes", "NoFullCycle" = "No")  # legend text
  ) +
  labs(
    x = "Time in REM (minutes)",
    y = "Recovery"
...
ggplot(PLPSG_sleep_stats_wider, aes(x = as.numeric(N3), y = as.numeric(R), color = recovery)) +
  geom_point(size = 3, alpha = 0.7) +
...
fullcycleplot = ggplot(PLPSG_sleep_stats_wider, aes(x = FullCycle, y = recovery, fill = FullCycle)) +
  ...
  labs(
    x = "Cycled Through All Stages of Sleep",
    y = "Mean Recovery ± SE"
  ) +
  theme_minimal() + 
  theme(text = element_text(family = "serif"),
...
ggarrange(fullcycleplot, fullcycleplot2,
          nrow = 1, ncol = 2, labels = c("A", "B"), common.legend = FALSE)

```

#interactions without individual data points 
```{r}
#interaction of N3 and full cycle
ggplot(PLPSG_sleep_stats_wider, aes(x = as.numeric(N3), y = recovery, color = FullCycle)) +
  geom_smooth(method = "lm", se = TRUE) +
...
ggplot(subset(PLPSG_sleep_stats_wider, condition == "sleep"), aes(x = (last_sleep_epoch != "N3"), y = recovery)) +
  geom_bar(stat = "summary", fun = "mean", position = "dodge", width = 0.6, color = "lightgrey") +
  geom_errorbar(
    stat = "summary", 
    fun.data = "mean_se", 
    width = 0.2, 
  ...
  theme_minimal() + 
  scale_x_discrete(label = c("SWS", "Other")) + 
